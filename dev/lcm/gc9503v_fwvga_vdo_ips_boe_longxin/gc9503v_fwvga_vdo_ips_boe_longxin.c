...
#define GC9504_TEST_lx_boot
unsigned int gc9503v_lx_flag=0;
// ---------------------------------------------------------------------------
//  Local Functions
// ---------------------------------------------------------------------------
...
static unsigned int lcm_compare_id(void)
{
 int array[4];
	...
	printf("longxin read hw id pin value = %d\n",hw_id);
    #if defined(BUILD_LK)
    ...
    ////// for id_auxadc end
 //   return 1;
    if(((0x9503 == id) || (0x9504 == id)) && (hw_id == 1))
...
// ---------------------------------------------------------------------------
LCM_DRIVER gc9503v_fwvga_vdo_ips_boe_longxin_lcm_drv = {
	.name			= "gc9503v_fwvga_vdo_ips_boe_longxin",
