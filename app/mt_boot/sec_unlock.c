...
{
	int ret = B_OK;
	int set_env_ret = B_OK;
	int err = PART_OK;

	set_env_ret = set_env("unlock_erase", "start");
	if (set_env_ret) {
	...
	/* if userdata is wiped successfully but for some
	 * reason md_udc is not wiped successfully, you'll
	 * need to enter recovery mode to wipe data again.
	 */
	if (partition_exists(METADATA_PARTITION) == PART_OK) { /* for /metadata */
		err = partition_erase(METADATA_PARTITION);
		if (err != PART_OK)
			goto data_wipe_fail;
	}

	if (partition_exists(METADATA_PARTITION_ALT) == PART_OK) { /* for /metadata */
		err = partition_erase(METADATA_PARTITION_ALT);
		if (err != PART_OK)
			goto data_wipe_fail;
	}

	if (partition_exists(CACHE_PARTITION) == PART_OK) { /* for /cache */
		err = partition_erase(CACHE_PARTITION);
		if (err != PART_OK)
			goto data_wipe_fail;
	}
...
data_wipe_fail:
	...
	set_env_ret = set_env("unlock_erase", "fail");
	if (set_env_ret)
		dprintf(CRITICAL,"set_env fail\n");
	return -1;
}
...
#define UNLOCK_CAP_RESP_MAX_SIZE 64
	...
	const char *unknown_fastboot_error_msg = "Unknown error\n";

	ret = fastboot_get_unlock_perm(&unlock_allowed);
	if (ret != B_OK) {
		if (snprintf(msg, UNLOCK_CAP_RESP_MAX_SIZE, "\nFailed to get unlock permission - Err:0x%x \n", ret) >= 0) {
			msg[UNLOCK_CAP_RESP_MAX_SIZE - 1] = '\0'; /* prevent msg from not being ended with '\0'*/
			fastboot_fail(msg);
		} else {
			fastboot_fail(unknown_fastboot_error_msg);
		}
	} else {
		if (snprintf(msg, UNLOCK_CAP_RESP_MAX_SIZE, "unlock_ability = %d", unlock_allowed) >= 0) {
			msg[UNLOCK_CAP_RESP_MAX_SIZE - 1] = '\0'; /* prevent msg from not being ended with '\0'*/
			fastboot_info(msg);
			fastboot_okay("");
		} else {
...
#define LKS_RESP_MAX_SIZE 64
	...
	if (ret != B_OK) {
		if (snprintf(msg, LKS_RESP_MAX_SIZE, "cannot get lks (ret = 0x%x)", ret) >= 0) {
			msg[LKS_RESP_MAX_SIZE - 1] = '\0';
			fastboot_fail(msg);
		} else {
			fastboot_fail(unknown_fastboot_error_msg);
		}
	} else {
		if (snprintf(msg, LKS_RESP_MAX_SIZE, "lks = %d", lock_state) >= 0) {
			msg[LKS_RESP_MAX_SIZE - 1] = '\0';
			fastboot_info(msg);
			fastboot_okay("");
		} else {
...
{
#define MAX_FASTBOOT_MSG_SZ (128)
	int ret = B_OK;
	char msg[MAX_FASTBOOT_MSG_SZ] = {0};
	const char *fastboot_error_msg = NULL;
	const char *unknown_fastboot_error_msg = "Unknown error\n";
	unsigned int unlock_allowed = 0;
	...
	while (1) {
		if (mtk_detect_key(MT65XX_MENU_SELECT_KEY)) { //VOL_UP
			...
			ret = fastboot_get_unlock_perm(&unlock_allowed);
			if (ret != B_OK) {
				if (snprintf(msg, MAX_FASTBOOT_MSG_SZ, "\nFailed to get unlock permission - Err:0x%x\n", ret) >= 0) {
					msg[MAX_FASTBOOT_MSG_SZ - 1] = '\0';
					fastboot_error_msg = msg;
				} else {
					fastboot_error_msg = unknown_fastboot_error_msg;
				}
				video_printf("Unlock failed...return to fastboot in 3s\n");
				mdelay(3000);
				fastboot_boot_menu();
				fastboot_fail(fastboot_error_msg);
				break;
			}
...
#define MAX_FASTBOOT_MSG_SZ (128)
	...
	char print_msg[MAX_FASTBOOT_MSG_SZ] = {0};
	const char *msg = NULL;
	int inFactory = 0;
	...
	if (ret) {
		msg = default_msg;
		if (snprintf(print_msg, MAX_FASTBOOT_MSG_SZ, "\nlock failed - Err:0x%x \n", ret) > 0) {
			msg = print_msg;
		}
