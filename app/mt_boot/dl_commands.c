...
void* special_heap_alloc(int length)
{
	void* buf = (void*)(u32)mblock_reserve_ext(&g_boot_arg->mblock_info,
		length, 0x10000, 0xC0000000, 0, "fastboot_heap");
...
bool cmd_download_standard(const char *arg, void *data, unsigned sz)
{
	...
	if(snprintf(response, MAX_RSP_SIZE, "DATA%08x", len) < 0)
        {
            fastboot_fail("ghost snprintf error.");
            return false;
        }
...
int _virtual_partition_support(const char *arg, void *data, unsigned int* p_sz, char **modified_arg)
{
#define TMP_DOWNLOAD_SIZE   52*1024*1024    //52MB, because kernel size(64M) - pagetable max size (8M) - ramdisk size(Estimate 4M)
	...
	*/
	...
	if (strlen(tmp) >= 5 && modified_arg != 0)
		*modified_arg = "boot";
	return 0;
...
bool cmd_flash_mmc_standard(const char *arg, void *data, unsigned sz)
{
	...
	while (remaining_len && (counter > 0)) {
		...
		if (sec_dl_permission_chk(arg, &permitted)) {
			if(snprintf(msg, sizeof(msg), "failed to get download permission for partition '%s'\n", arg) < 0)
			{
				fastboot_fail("ghost snprintf error.");
				return false;
			}
		...
		if (0 == permitted) {
			if(snprintf(msg, sizeof(msg), "download for partition '%s' is not allowed\n", arg) < 0)
			{
				fastboot_fail("ghost snprintf error.");
				return false;
			}
		...
		TIME_START;
                char *modified_arg = 0;
		if (_virtual_partition_support(arg, data, &sz, &modified_arg) < 0) {
                        set_response_msg("virtual partition write fail");
			return false;
		}
		else { /* is boot virtual partition, ex:ramdisk */
			if (remaining_len < sz)
				remaining_len = sz;
		}
	...
	if (!strcmp(arg, "NOR")) {
...
#endif
		{
			...
			if (sec_format_permission_chk(arg, &permitted)) {
				if(snprintf(msg, sizeof(msg), "failed to get format permission for partition '%s'\n", arg) < 0)
				{
					fastboot_fail("ghost snprintf error.");
					return;
				}
			...
			if (0 == permitted) {
				if(snprintf(msg, sizeof(msg), "format for partition '%s' is not allowed\n", arg) < 0)
				{
					fastboot_fail("ghost snprintf error.");
					return;
				}
...
void cmd_oem_ultra_flash(const char *arg, void *data, unsigned sz)
{
	...
	{
		if(snprintf(response, MAX_RSP_SIZE, "Ultra-flash Disabled. [%s]", fastboot_get_var("max-download-size")) < 0)
		{
			fastboot_fail("ghost sprintf error.");
			return;
		}
	...
	if(part == 0)
	{
		fastboot_fail("ultra flash arg is null.");
		return;
	}
	...
	strncpy(partition_name_history, part, MAX_PARTITION_NAME_LEN-1);
	partition_name_history[MAX_PARTITION_NAME_LEN-1] = 0;

	switch_max_download_len((partition_name_history_len != 0) && (!is_special_partition(partition_name_history)));

	if(snprintf(response, MAX_RSP_SIZE, "%s. [%s]", partition_name_history, fastboot_get_var("max-download-size")) < 0)
	{
		fastboot_fail("name too long.");
		return;
	}
...
void cmd_oem_ultra_flash_en(const char *arg, void *data, unsigned sz)
{
	...
	else if (!strncmp(arg, " 0", strlen(" 0")))
	{
		...
		if(snprintf(response, MAX_RSP_SIZE, "\tCurrent ultra-flash setting:%s",
			g_ultra_flash_enable ? "Enabled" : "Disabled") < 0)
		{
			fastboot_fail("ghost snprintf error.");
			return;
		}
...
bool cmd_download_ultra(const char *arg, void *data, unsigned sz)
{
	...
	if(snprintf(response, MAX_RSP_SIZE, "DATA%08llx", len) < 0)
	{
		fastboot_fail("ghost snprintf error.");
		return false;
	}
	...
	strncpy((char*)part_info.name, partition_name_history, MAX_PARTITION_NAME_LEN-1);
	...
	if(FAIL(status))
	{
		if(snprintf(response, MAX_RSP_SIZE, "Transmission FAIL: [%d]", status) < 0)
		{
			fastboot_fail("ghost snprintf error.");
			return false;
		}
