...
int disp_lcm_get_dts_panel_index()
{
	return dts_panel_index;
}

static int dts_is_cphy = 0;
void disp_lcm_set_dts_phy_type(int phy_type)
{
	dts_is_cphy = phy_type;
}

int disp_lcm_fill_lcm_dts_phandle(const void* fdt, int idx)
{
    int nodeoffset, len, data, phandle, nodeoffset1;
    char *name, *value;
    struct fdt_property *prop;
    const void *val;
    char *val1;
    char buf[100];
    char *phy_type = "mediatek,mt6873-mipi-tx-cphy";

	snprintf(buf, sizeof(buf) - 1, "/dsi/panel%d/port/endpoint", idx);
	nodeoffset = fdt_path_offset(fdt, buf);
	if (nodeoffset < 0) {
		DISPERR("[%s] get panel node failed\n", __func__);
		goto error;
	}
	...
	if (val) {
		...
		DISPMSG("default dsi remote-endport is 0x%x before lk set \n", data);
	} else {
		DISPERR("default dsi remote-endport in dtb is empty\n");
	}

	phandle = cpu_to_fdt32(phandle);
	fdt_setprop(fdt, nodeoffset, "remote-endpoint", &phandle, sizeof(phandle));

	val = fdt_getprop(fdt, nodeoffset, "remote-endpoint", NULL);
	if (val) {
		memcpy(&data, val, sizeof(data));
		data = fdt32_to_cpu(data);
		DISPMSG("update dsi remote-endport to 0x%x after lk set \n", data);
	} else {
	...
	if (dts_is_cphy) {
		...
		if (val1) {
			DISPMSG("set dsi mipi tx data as %s befor \n", val1);
		} else {
			DISPERR("dsi mipi tx data is empty befor set \n");
		}
		fdt_setprop_string(fdt, nodeoffset1, "compatible", phy_type);
		val1 = fdt_getprop(fdt, nodeoffset1, "compatible", NULL);
		if (val1) {
...
int disp_lcm_get_dts_u32(const void* fdt, char* path, char* name)
{
    int nodeoffset, phandle, len;
	...
	if (nodeoffset < 0) {
		DISPERR("[%s] get \"%s\" failed\n", __func__, path);
		goto error;
	}

	val = fdt_getprop(fdt, nodeoffset, name, &len);
	if (len > 0 && val) {
		//memcpy(&data, val, sizeof(data));
		data = fdt32_to_cpu(*val);
		DISPMSG("\"%s/%s\"= %u, val=%u, len=%d\n", path, name, data, *val, len);
	} else {
		DISPERR("\"%s/%s\" is empty, len:%d\n", path, name, len);
	}
	return data;
		...
		if (_lcm_count() == 0) {
			...
			if (plcm_name == NULL) {
				...
				for (i=0; i<_lcm_count(); i++) {
					...
					DISPDBG("we will check lcm: %s, i=%d\n", lcm_drv->name, i);
					if (lcm_id == LCM_INTERFACE_NOTDEFINED ||(lcm_id != LCM_INTERFACE_NOTDEFINED && plcm->lcm_if_id == lcm_id)) {
						...
						if (lcm_drv->compare_id != NULL) {
							if (lcm_drv->compare_id() != 0) {
								...
								if (lk_drv_fdt)
									target_fps = disp_lcm_get_dts_u32(lk_drv_fdt, "/dsi", "switch_fps");
								if (target_fps > 0) {
									if (hlcm_param.dsi.fps != target_fps) {
										DISPMSG("lcm:%s found with fps:%u didn't match target fps:%u\n",
											lcm_drv->name, hlcm_param.dsi.fps, target_fps);
										_display_interface_path_deinit(handle);
										continue;
									} else {
										DISPMSG("lcm:%s found with fps:%u, target fps:%u\n",
											lcm_drv->name, hlcm_param.dsi.fps, target_fps);
									}
								}
#endif
