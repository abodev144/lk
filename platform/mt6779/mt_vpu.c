...
exit:
	...
	if (!ret) {
		...
		/* cache operation before config EMIMPU */
		arch_sync_cache_range((addr_t)dstBase, VPU_DRAM_SIZE);

		region_info.start = (unsigned long long)dstBase;
		region_info.end = (unsigned long long)(dstBase + VPU_DRAM_SIZE - 0x1);
		dprintf(INFO, "[%s] prot region [0x%X:0x%X]\n", MODULE_NAME, dstBase, VPU_DRAM_SIZE);
