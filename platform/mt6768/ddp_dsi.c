...
void DSI_Config_VDO_Timing(DISP_MODULE_ENUM module,
	void *cmdq, LCM_DSI_PARAMS *dsi_params)
{
	int i = 0;
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		if (bdg_is_bdg_connected() == 1) {
			...
			DSI_OUTREG32(cmdq, DSI_REG_BASE[i] +
				DISP_REG_DSI_VSA_NL,
			...
			*/
			...
			horizontal_frontporch_byte =
			    (dsi_params->horizontal_frontporch *
				dsiTmpBufBpp - 12);
			horizontal_bllp_byte = (dsi_params->horizontal_bllp *
				dsiTmpBufBpp);
...
enum DSI_STATUS DSI_TXRX_Control(DISP_MODULE_ENUM module, void* cmdq, LCM_DSI_PARAMS *dsi_params)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		DISP_REG_SET_FIELD(cmdq, DSI_TXRX_CON_FLD_LANE_NUM,
			DSI_REG_BASE[i] +
			DISP_REG_DSI_TXRX_CON, lane_num_bitvalue);
...
void DSI_PHY_TIMCONFIG(DISP_MODULE_ENUM module, void* cmdq, LCM_DSI_PARAMS *dsi_params)
{
	...
	*/
	...
	if (bdg_is_bdg_connected() == 1) {
		...
		/* clk_hs_prep = 80ns */
		timcon3.CLK_HS_PRPR =
			NS_TO_CYCLE(80, cycle_time);

		/* clk_zero + clk_hs_prep > 300ns (spec) */
		/* clk_zero = 400ns - clk_hs_prep */
		timcon2.CLK_ZERO =
			NS_TO_CYCLE(400, cycle_time) - timcon3.CLK_HS_PRPR;
...
int ddp_dsi_config(DISP_MODULE_ENUM module, disp_ddp_path_config *config, void *cmdq_handle)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		_copy_dsi_params(&(config->dsi_config),
	...
	if (bdg_is_bdg_connected() == 1) {
		...
		DSI_Config_VDO_Timing(module, NULL,
			&(config->dsi_config));
