...
#ifdef SW_EARLY_PORTING
#else
	ret = spislv_write_register_mask(addr, value, msk);
#endif
	return ret;
}
			...
			void *cmdq, LCM_DSI_PARAMS *tx_params)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		mtk_spi_mask_write((u32)(DISPSYS_BDG_TX_DSI0_BASE
			+ REG_DSI_TX_PSCON),
			DSI_TX_FLD_DSI_PS_WC, ps_wc * line_back_to_LP);
		mtk_spi_mask_write((u32)(DISPSYS_BDG_TX_DSI0_BASE
			+ REG_DSI_TX_PSCON),
		...
		mtk_spi_mask_write((u32)(DISPSYS_BDG_TX_DSI0_BASE
			+ REG_DSI_TX_SIZE_CON),
			DSI_TX_SIZE_FLD_DSI_WIDTH, width * line_back_to_LP);
		mtk_spi_mask_write((u32)(DISPSYS_BDG_TX_DSI0_BASE
			+ REG_DSI_TX_SIZE_CON),
...
#else
			DSI_OUTREG32(cmdq, TX_REG[i]->DSI_TX_BUF_CON1, 0x0dfd0660);
#endif
		}
	}
...
int bdg_tx_start(enum DISP_BDG_ENUM module, void *cmdq)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		mtk_spi_write(DISPSYS_BDG_TX_DSI0_BASE + REG_DSI_TX_START, 0);
		mtk_spi_write(DISPSYS_BDG_TX_DSI0_BASE + REG_DSI_TX_START, 1);
	}
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		if (tmp != 0) {
			...
			if (tmp & (1 << 16)) {
				...
				DISPMSG("transmission duration is more than the programmed time.\n");
			}
			if (tmp & (1 << 17)) {
		...
		if (tmp != 0) {
			...
			if (tmp & (1 << 0)) {
				...
				DISPMSG("not accepting the incoming data during programmed value.\n");
			}
			if (tmp & (1 << 1)) {
		   ...
		   disp_ddp_path_config *config, void *cmdq)
{
	...
	DISPMSG("%s, data_rate=%d, bdg_ssc_disable=%d, ssc_disable=%d, dsc_enable=%d, bdg_tx_mode=%d\n",
		__func__, tx_params->data_rate, tx_params->bdg_ssc_disable,
			...
			void *cmdq, LCM_DSI_PARAMS *tx_params)
{
	...
	unsigned int val;
	LCM_DSC_CONFIG_PARAMS *params = &tx_params->dsc_params;
	...
	mtk_spi_mask_write((u32)(DISPSYS_BDG_DISP_DSC_BASE +
			REG_DISP_DSC_CON),
			DISP_DSC_CON_FLD_DSC_UFOE_SEL, 1);
	/* DSC Empty flag always high */
	mtk_spi_mask_write((u32)(DISPSYS_BDG_DISP_DSC_BASE +
			REG_DISP_DSC_CON),
			DISP_DSC_CON_FLD_DSC_EMPTY_FLAG_SEL, 1);
	...
	if ((params->ver & 0xf) == 2)
		mtk_spi_mask_write((u32)(DISPSYS_BDG_DISP_DSC_BASE +
			REG_DISP_DSC_SHADOW),
			DISP_DSC_VERSION_MINOR, 0x2); /* DSC V1.2 */
	else
		mtk_spi_mask_write((u32)(DISPSYS_BDG_DISP_DSC_BASE +
			REG_DISP_DSC_SHADOW),
			DISP_DSC_VERSION_MINOR, 0x1); /* DSC V1.1 */
			...
			disp_ddp_path_config *config, void *cmdq)
{
	...
	/* set 0x1b for MMCLK 270M set 0x37 for MMCLK 405M */
