...
unsigned int data_phy_cycle = 0;
unsigned int deskew_done = 0;
#define _BDG_CMD_MODE_
#endif
...
#ifdef CONFIG_MTK_MT6382_BDG
void DSI_Config_VDO_Timing_with_DSC(DISP_MODULE_ENUM module,
	void *cmdq, LCM_DSI_PARAMS *dsi_params)
{
	...
	unsigned int t_hbllp, ps_wc;
	unsigned int ap_tx_total_word_cnt_no_hfp_wc;
	unsigned int ap_tx_total_word_cnt;
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		ap_tx_total_word_cnt =
			(get_bdg_line_cycle() * lanes * RXTX_RATIO + 99) / 100;
		...
		DISPMSG(
			...
			__func__, ps_wc, get_bdg_line_cycle(),
			ap_tx_total_word_cnt,
			data_phy_cycle, ap_tx_total_word_cnt_no_hfp_wc);

		DISPMSG(
			"[DISP]-kernel-%s, mode=0x%x, line_back_to_LP=%d, t_vsa=%d, t_vbp=%d, t_vfp=%d, t_hsa=%d, t_hbp=%d, t_hfp=%d, t_hbllp=%d\n",
			__func__, dsi_params->mode, line_back_to_LP, t_vsa, t_vbp, t_vfp,
		...
		ap_tx_cycle_time = 8000 * get_bdg_line_cycle() /
			get_bdg_data_rate() / ap_tx_line_cycle;

		DISPMSG(
			"[DISP]-kernel-%s, ap_tx_total_word_cnt=%d, ap_tx_line_cycle=%d, ap_tx_cycle_time=%d\n",
			__func__, ap_tx_total_word_cnt,
			ap_tx_line_cycle, ap_tx_cycle_time);
...
void DSI_DPHY_Calc_VDO_Timing(DISP_MODULE_ENUM module, void* cmdq, LCM_DSI_PARAMS *dsi_params)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		DSI_OUTREG32(cmdq, DSI_REG_BASE[i] + DISP_REG_DSI_VACT_NL, dsi_params->vertical_active_line / line_back_to_LP);
...
enum DSI_STATUS DSI_PS_Control(DISP_MODULE_ENUM module, void* cmdq, LCM_DSI_PARAMS *dsi_params, int w, int h)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		DISP_REG_SET_FIELD(cmdq, DSI_VACT_NL_FLD_VACT_NL, DSI_REG_BASE[i] + DISP_REG_DSI_VACT_NL, h / line_back_to_LP);
		...
		DISP_REG_SET_FIELD(cmdq, DSI_PSCON_FLD_DSI_PS_WC,
				DSI_REG_BASE[i] + DISP_REG_DSI_PSCON, ps_wc * line_back_to_LP);
		DISP_REG_SET_FIELD(cmdq, DSI_PSCON_FLD_DSI_PS_SEL,
				DSI_REG_BASE[i] + DISP_REG_DSI_PSCON, ps_sel_bitvalue);
		DSI_OUTREG32(cmdq, DSI_REG_BASE[i] + DISP_REG_DSI_SIZE_CON,
...
void DSI_DPHY_TIMCONFIG(DISP_MODULE_ENUM module, void* cmdq, LCM_DSI_PARAMS *dsi_params)
{
	...
	/*div2_real=div2 ? div2*0x02 : 0x1;
	* cycle_time = (1000 * div2 * div1 * pre_div * post_div/
	* (fbk_sel * (fbk_div+0x01) * 26) + 1;
	* ui = (1000 * div2 * div1 * pre_div * post_div)/
	* (fbk_sel * (fbk_div+0x01) * 26 * 2) + 1;/
	*/
...
#define NS_TO_CYCLE1(n, c)	((n) / (c) + (((n) % (c)) ? 1 : 0))
	...
	/* lpx >= 50ns (spec) */
	/* lpx = 60ns */
	timcon0.LPX = NS_TO_CYCLE1(60, cycle_time);
	if (timcon0.LPX < 2)
		timcon0.LPX = 2;

	/* hs_prep = 40ns+4*UI ~ 85ns+6*UI (spec) */
	/* hs_prep = 64ns+5*UI */
	timcon0.HS_PRPR = NS_TO_CYCLE1((64 + 5 * ui), cycle_time) + 1;

	/* hs_zero = (200+10*UI) - hs_prep */
	timcon0.HS_ZERO = NS_TO_CYCLE1((200 + 10 * ui), cycle_time);
	timcon0.HS_ZERO =
	...
	/* clk_hs_prep = 80ns */
	timcon3.CLK_HS_PRPR = NS_TO_CYCLE1(80, cycle_time);

	/* clk_zero + clk_hs_prep > 300ns (spec) */
	/* clk_zero = 400ns - clk_hs_prep */
	timcon2.CLK_ZERO = NS_TO_CYCLE1(400, cycle_time) -
				timcon3.CLK_HS_PRPR;
	...
	/* clk_trail > 60ns (spec) */
	/* clk_trail = 100ns */
	timcon2.CLK_TRAIL = NS_TO_CYCLE1(100, cycle_time) + 1;
	if (timcon2.CLK_TRAIL < 2)
		timcon2.CLK_TRAIL = 2;
	...
	/* clk_post = 96ns+52*UI */
	timcon3.CLK_HS_POST = NS_TO_CYCLE1((96 + 52 * ui), cycle_time);
#else
	hs_trail_m=1;
...
#ifdef CONFIG_MTK_MT6382_BDG
static void DSI_send_read_cmd_via_bdg(void *cmdq,
			DISP_MODULE_ENUM module, bool hs,
			UINT8 cmd)
{
	int dsi_i = 0;
	struct DSI_T0_INS t0;
...
static void DSI_send_return_size_cmd(void *cmdq,
			DISP_MODULE_ENUM module, bool hs,
			UINT8 buffer_size)
{
	int dsi_i = 0;
...
static int check_rdrdy_cmddone_irq(void *cmdq, DISP_MODULE_ENUM module)
{
	int dsi_i, i = 0;
	...
	if (DSI_REG[dsi_i]->DSI_INTSTA.RD_RDY != 0 ||
		...
		DISP_REG_SET_FIELD(cmdq, DSI_INTEN_FLD_LPRX_RD_RDY_INT_EN,
			DSI_REG_BASE[dsi_i] + DISP_REG_DSI_INTEN, 0);
...
UINT32 DSI_dcs_read_lcm_reg_via_bdg(DISP_MODULE_ENUM module,
			       void *cmdq, UINT8 cmd,
			       UINT8 *buffer, UINT8 buffer_size)
{
	int dsi_i = 0;
	...
	do {
		if (max_try_count == 0) {
			...
			DISP_REG_SET_FIELD(cmdq,
				DSI_INTEN_FLD_LPRX_RD_RDY_INT_EN,
				DSI_REG_BASE[dsi_i] + DISP_REG_DSI_INTEN, 0);
			return 0;
		}
		...
		timeout = 5000;
		while (timeout) {
			status = INREG32(DSI_REG_BASE[dsi_i] +
				DISP_REG_DSI_INTSTA);
			if (timeout % 1000 == 0)
		...
		if (timeout == 0) {
			...
			DISP_REG_SET_FIELD(cmdq, DSI_RX_RACK_FLD_RACK,
				DSI_REG_BASE[dsi_i] + DISP_REG_DSI_RX_RACK, 1);
			DSI_Reset(module, NULL);
			DISP_REG_SET_FIELD(cmdq,
				DSI_INTEN_FLD_LPRX_RD_RDY_INT_EN,
				DSI_REG_BASE[dsi_i] + DISP_REG_DSI_INTEN, 0);
...
UINT32 DSI_dcs_read_lcm_reg_v2(DISP_MODULE_ENUM module,
	void *cmdq, UINT8 cmd, UINT8 *buffer, UINT8 buffer_size)
{
#ifdef CONFIG_MTK_MT6382_BDG
	UINT32 ret = 0;
	...
	ret = DSI_dcs_read_lcm_reg_via_bdg(module, cmdq, cmd,
		buffer, buffer_size);
		...
		do {
			...
			if (DSI_REG[d]->DSI_INTEN.RD_RDY == 0) {
				DISP_REG_SET_FIELD(cmdq,
					DSI_INTEN_FLD_LPRX_RD_RDY_INT_EN,
					DSI_REG_BASE[d] +
					DISP_REG_DSI_INTEN, 1);
			}
			if (DSI_REG[d]->DSI_INTEN.CMD_DONE == 0) {
				DISP_REG_SET_FIELD(cmdq,
					DSI_INTEN_FLD_CMD_DONE_INT_EN,
					DSI_REG_BASE[d] +
					DISP_REG_DSI_INTEN, 1);
			}
...
#ifdef CONFIG_MTK_MT6382_BDG
static void DSI_config_bdg_reg(void *cmdq,
...
void DSI_send_cmdq_to_bdg(DISP_MODULE_ENUM module, void *cmdq,
		     unsigned int cmd, unsigned char count,
...
#ifdef CONFIG_MTK_MT6382_BDG
	config->dsi_config.data_rate = get_ap_data_rate();
#endif
	DISPMSG(
		"%s, PLL_CLOCK=%d, data_rate=%d, mode=%d\n",
		__func__, config->dsi_config.PLL_CLOCK,
		config->dsi_config.data_rate, config->dsi_config.mode);
...
#ifdef CONFIG_MTK_MT6382_BDG
		if (config->dsi_config.bdg_dsc_enable) {
			DSI_Config_VDO_Timing_with_DSC(module, NULL,
				&(config->dsi_config));
		} else
#endif
		{
...
#ifdef CONFIG_MTK_MT6382_BDG
	if (get_ap_data_rate() > 1700 && deskew_done == 0)
		DSI_MIPI_deskew(module, NULL);
#endif
...
#ifdef CONFIG_MTK_MT6382_BDG
void DSI_MIPI_deskew(DISP_MODULE_ENUM module, void *cmdq)
{
	unsigned int i = 0;
	...
	DISPFUNCSTART();
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		phy_syncon = INREG32(DSI_REG_BASE[i] + DISP_REG_DSI_PHY_SYNCON);

		DSI_OUTREG32(cmdq, &DSI_REG[i]->DSI_PHY_SYNCON, 0x00aaffff);
		DISP_REG_SET_FIELD(cmdq, DSI_TIME_CON0_FLD_SKEWCAL_PRD,
			DSI_REG_BASE[i] + DISP_REG_DSI_TIME_CON0, 6);

		DSI_OUTREG32(cmdq, &DSI_REG[i]->DSI_START, 0);

		DSI_WaitForNotBusy(module, cmdq);
		DISP_REG_SET_FIELD(cmdq, DSI_PHY_SYSCON_FLD_HS_DB_SYNC_EN,
			DSI_REG_BASE[i] + DISP_REG_DSI_PHY_SYNCON, 1);
		DISP_REG_SET_FIELD(cmdq, DSI_PHY_TIMCON2_FLD_DA_HS_SYNC,
			DSI_REG_BASE[i] + DISP_REG_DSI_PHY_TIMCON2, 2);
		DISP_REG_SET_FIELD(cmdq, DSI_INTEN_FLD_SKEWCAL_DONE_INT_EN,
			DSI_REG_BASE[i] + DISP_REG_DSI_INTSTA, 0);
		DISP_REG_SET_FIELD(cmdq, DSI_START_FLD_SKEWCAL_START,
		...
		DISP_REG_SET_FIELD(cmdq, DSI_PHY_TIMCON2_FLD_DA_HS_SYNC,
			DSI_REG_BASE[i] + DISP_REG_DSI_PHY_TIMCON2, 1);
		deskew_done = 1;
		if (timeout == 0) {
