...
static void set_gear_id_list()
{
	...
	ret = set_env("md1_ccb_gear_list",
		"1(2,20);2(2,10);3(0,0);11(2,2);6(2,50)");
...
static int cal_share_mem_layout(int load_flag)
{
	...
	for (i = 0; i < sizeof(csmem_layout)/sizeof(csmem_item_t); i++) {
		...
		ALWAYS_LOG("%s:cacheable_buf_sz=0x%x,item=%d,size=0x%x\n",
			__func__, cacheable_buf_sz, csmem_layout[i].item_cnt, csmem_layout[i].csmem_buffer_size);
	}

	if (cacheable_buf_sz != 0)
		cacheable_buf = ccci_request_named_mem("ap_md_c_smem", cacheable_buf_sz);
	...
	if (cacheable_buf_sz == NULL) {
		...
		/* CCB in slot3,4 */
		md1_bank4_cache_offset = 96 * 1024 * 1024;;
...
static const mpu_att_t mpu_att_default[MPU_REGION_ID_TOTAL_NUM][MPU_MDOMAIN_ID_TOTAL_NUM] = {
	...
	/*21*/{}, /* VPU */
	/*22*/{
		NO_PROTECTION,  SEC_R_NSEC_R,  [2 ... 3] = FORBIDDEN, NO_PROTECTION, NO_PROTECTION, SEC_R_NSEC_RW,
		[7 ... 15] = FORBIDDEN
	},
...
int plat_apply_platform_setting(int load_md_flag)
{
	...
	if (load_md_flag & (1<<MD_SYS1)) {
		...
		/* remapping CCB to slot3,4 in bank4 */
		md_bank4_remapping_by_slot(MD_SYS1, csmem_info.csmem_buffer_addr, 3);
		md_bank4_remapping_by_slot(MD_SYS1, csmem_info.csmem_buffer_addr + 0x2000000, 4);
