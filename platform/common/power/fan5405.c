...
int gpio_number   = GPIO154  | 0x80000000; // GPIO64  | 0x80000000
int gpio_off_mode = GPIO_MODE_00;
int gpio_on_mode  = GPIO_MODE_00;
#else
int gpio_number   = (154 | 0x80000000); 
int gpio_off_mode = 0;
int gpio_on_mode  = 0;
#endif
...
/**********************************************************
  ...
  *
  *********************************************************/
#define fan5405_SLAVE_ADDR_WRITE   0xd4  //0xD4
#define fan5405_SLAVE_ADDR_Read    0xd5   //0xD5
#define fan5405_SLAVE_ADDR    0x6a   //0xD5
//***********************************
struct fan5405_info {
...
int fan5405_dump_register(struct mtk_charger_info *mchr_info);
int fan5405_charging_enable(struct mtk_charger_info *mchr_info, bool enable);
//BSP:add XLEWHLES-25 by ronghe.cheng 20181108 start
//int fan5405_reset_watch_dog_timer(struct mtk_charger_info *mchr_info, bool enable);
static struct mtk_charger_ops fan5405_mchr_ops = {
	.dump_register = fan5405_dump_register,
	.enable_charging = fan5405_charging_enable,
};
//BSP:add XLEWHLES-25 by ronghe.cheng 20181108 end
static struct fan5405_info g_fan5405_info = {
	.mchr_info = {
		.name = "primary_charger",
		.alias_name = "fan5405",
		.device_id = -1,
		.mchr_ops = &fan5405_mchr_ops,
	},
	.i2c = {
		.id = I2C6,
		.addr = fan5405_SLAVE_ADDR,
		.mode = ST_MODE,
		.speed = 100,
	},
	.i2c_log_level = INFO,
...
int fan5405_dump_register(struct mtk_charger_info *mchr_info)
{
	...
	printf("lk lk lk [fan5405] "); 
    for (i=0;i<fan5405_REG_NUM;i++)
    {
        fan5405_read_byte(i, &fan5405_reg[i]);
        printf("fan5405[0x%x]=0x%x ", i, fan5405_reg[i]);        
    }
	printf("\n");
return 0;	
}
int fan5405_charging_enable(struct mtk_charger_info *mchr_info, bool enable)
{
	...
	if(enable){
		...
		fan5405_reg_config_interface(0x01,0xf8);   //pj debug
		//fan5405_reg_config_interface(0x02,0x8e);
		fan5405_reg_config_interface(0x05,0x04);
		fan5405_reg_config_interface(0x04,0x5A); // 5A  1.46A  
	}

	if (KAL_TRUE == enable){
		mt_set_gpio_mode(gpio_number,gpio_on_mode);  
		mt_set_gpio_dir(gpio_number,gpio_on_dir);
		mt_set_gpio_out(gpio_number,GPIO_OUT_ZERO);
	}else
	{
		mt_set_gpio_mode(gpio_number,gpio_on_mode);  
		mt_set_gpio_dir(gpio_number,gpio_on_dir);
		mt_set_gpio_out(gpio_number,GPIO_OUT_ONE);
	}
	return 0;
}
...
{
    fan5405_read_byte(i, &fan5405_reg[i]);
    printf("lk lk lk fan5405[0x%x]=0x%x\n", i, fan5405_reg[i]); 
}
...
void fan5405_turn_on_charging(void)
{
    ...
    fan5405_reg_config_interface(0x05,0x04);
    fan5405_reg_config_interface(0x04,0x5A); // 5A  1.46A  
	printf("lk lk lk fan5405_turn_on_charging\n");
}

void fan5405_hw_init(void)
{
#if defined(HIGH_BATTERY_VOLTAGE_SUPPORT)
    printf("lk lk lk fan5405_hw_init (0x06,0x49)\n");
    fan5405_reg_config_interface(0x06,0x49); // set ISAFE and HW CV point (4.34)
    fan5405_reg_config_interface(0x02,0x94); // 4.34
#else
    printf("lk lk lk fan5405_hw_init (0x06,0x40)\n");
    fan5405_reg_config_interface(0x06,0x40); // set ISAFE
    fan5405_reg_config_interface(0x02,0x8e); // 4.2
#endif    
}

int fan5405_chg_probe(void)
{
		int ret = 0;
	fan5405_hw_init();
			mtk_charger_set_info(&(g_fan5405_info.mchr_info));
		return ret;
}
