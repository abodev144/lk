...
static int yellow_state_result(void)
{
...
static int orange_state_result(void)
{
	int ret = 0;
...
static int red_state_result(void)
{
	int ret = -1;
...
int vboot_state_result(uint32_t bootimg_type)
{
	...
	switch (g_boot_state) {
	case BOOT_STATE_ORANGE:
		ret = orange_state_result();
		break;
	case BOOT_STATE_YELLOW:
#ifdef MTK_SECURITY_YELLOW_STATE_SUPPORT
		ret = yellow_state_result();
		if (0 == ret) /* user confirms to boot into yellow state */
			break;
		/* fall into red state if user refuses to enter yellow state */
#else
