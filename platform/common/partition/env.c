...
void env_init(void)
{
	...
	for (area = 0; area < SYSENV_AREA_MAX; area++) {
		...
		g_env[area].sig[sizeof(g_env[area].sig) -1] = '\0';

		memcpy(g_env[area].sig_1,env_buffer[area]+CFG_ENV_SIG_1_OFFSET,sizeof(g_env[area].sig_1));
		g_env[area].sig_1[sizeof(g_env[area].sig_1) -1] = '\0';
...
static int write_env_area(char *env_buf, unsigned int area)
{
	...
	env_buf[CFG_ENV_SIG_1_OFFSET + count -1] = '\0';
...
int set_env_special_bits(uint8_t po, uint8_t val)
{
    int ret = 0, len;
    size_t count;
    off_t offset;
    
    ret = read_env_area(env_buffer[SYSENV_RW_AREA], SYSENV_RW_AREA);
    if (ret < 0) {
        dprintf(CRITICAL, "[%s]read_env_area %d fail, ret = %d\n", MODULE_NAME, SYSENV_RW_AREA, ret);
        return ret;
    }

    memcpy(g_env[SYSENV_RW_AREA].sig, env_buffer[SYSENV_RW_AREA], sizeof(g_env[SYSENV_RW_AREA].sig));
    g_env[SYSENV_RW_AREA].sig[7] = (0x1 << po) & val;
    memcpy(env_buffer[SYSENV_RW_AREA], g_env[SYSENV_RW_AREA].sig, sizeof(g_env[SYSENV_RW_AREA].sig));
    ret = write_env_area(env_buffer[SYSENV_RW_AREA], SYSENV_RW_AREA);
    if (ret < 0) {
        dprintf(CRITICAL, "read_env_area %d fail, ret = %d\n", SYSENV_RW_AREA, ret);
        return ret;
    }
    return 0;
}

int get_env_special_bits(uint8_t po)
{
    int ret;
    ...
    memcpy(g_env[SYSENV_RW_AREA].sig, env_buffer[SYSENV_RW_AREA], sizeof(g_env[SYSENV_RW_AREA].sig));
    ret = g_env[SYSENV_RW_AREA].sig[7] & (0x1 << po);
