...
#define ADC_SET_BITS(REG, BS)       ((*(volatile unsigned int*)(REG)) |= (unsigned int)(BS))
#define ADC_CLR_BITS(REG, BS)       ((*(volatile unsigned int*)(REG)) &= ~((unsigned int)(BS)))
void mt_ADC_power_on()
{
	ADC_SET_BITS(CLR_CLK_GATING_CTRL1, (1<<MT_CG_ADC_SW_CG));

	// ADCDBG("enable_clock ADC\n");
	mdelay(100);
}
